...
// Helper function to create a clean, serializable torrent object
function cleanTorrentData(torrent) {
  ...
  try {
    ...
    if (activeTorrents.has(torrentId)) {
      ...
      return res.json({ 
        ...
        torrent: cleanTorrentData(existingTorrent) // Use the cleaning function
      });
    }
    ...
    res.json({ torrentId, torrentHash: normalizedHash, status: 'adding', torrent: cleanTorrentData(torrentInfo) });
...
app.get('/torrent/:id', (req, res) => {
  ...
  res.json(cleanTorrentData(torrent));
});

// List all torrents
app.get('/torrents', (req, res) => {
  const torrents = Array.from(activeTorrents.values()).map(cleanTorrentData);
  res.json(torrents);
});
